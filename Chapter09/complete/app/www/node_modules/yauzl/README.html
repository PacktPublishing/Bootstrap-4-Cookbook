  ...
  <div class="collapse navbar-collapse" id="navbarsExampleDefault">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item active">
        ...
        <h1>yauzl</h1><p><a href="https://travis-ci.org/thejoshwolfe/yauzl"><img src="https://travis-ci.org/thejoshwolfe/yauzl.svg?branch=master" alt="Build Status"></a>
...
<li>Catch unsafe filenames entries.
A zip file entry throws an error if its file name starts with <code>&quot;/&quot;</code> or <code>/[A-Za-z]:\//</code>
or if it contains <code>&quot;..&quot;</code> path segments or <code>&quot;\\&quot;</code> (per the spec).</li>
</ul>
<h2>Usage</h2><pre><code class="language-js">var yauzl = require(&quot;yauzl&quot;);
var fs = require(&quot;fs&quot;);
...
<h2>API</h2><p>The default for every optional <code>callback</code> parameter is:</p>
<pre><code class="language-js">function defaultCallback(err) {
  if (err) throw err;
}
</code></pre>
<h3>open(path, [options], [callback])</h3><p>Calls <code>fs.open(path, &quot;r&quot;)</code> and gives the <code>fd</code>, <code>options</code>, and <code>callback</code> to <code>fromFd()</code> below.</p>
<p><code>options</code> may be omitted or <code>null</code>. The defaults are <code>{autoClose: true, lazyEntries: false}</code>.</p>
<p><code>autoClose</code> is effectively equivalent to:</p>
<pre><code class="language-js">zipfile.once(&quot;end&quot;, function() {
  zipfile.close();
});
</code></pre>
...
<h3>fromFd(fd, [options], [callback])</h3><p>Reads from the fd, which is presumed to be an open .zip file.
...
An <code>err</code> is provided if the End of Central Directory Record Signature cannot be found in the file,
which indicates that the fd is not a zip file.
<code>zipfile</code> is an instance of <code>ZipFile</code>.</p>
<p><code>options</code> may be omitted or <code>null</code>. The defaults are <code>{autoClose: false, lazyEntries: false}</code>.
See <code>open()</code> for the meaning of the options.</p>
<h3>fromBuffer(buffer, [options], [callback])</h3><p>Like <code>fromFd()</code>, but reads from a RAM buffer instead of an open file.
<code>buffer</code> is a <code>Buffer</code>.
<code>callback</code> is effectively passed directly to <code>fromFd()</code>.</p>
...
<p><code>options</code> may be omitted or <code>null</code>. The defaults are <code>{lazyEntries: false}</code>.
See <code>open()</code> for the meaning of the options.
The <code>autoClose</code> option is ignored for this method.</p>
<h3>fromRandomAccessReader(reader, totalSize, [options], [callback])</h3><p>This method of creating a zip file allows clients to implement their own back-end file system.
...
<p><code>options</code> may be omitted or <code>null</code>. The defaults are <code>{autoClose: true, lazyEntries: false}</code>.
See <code>open()</code> for the meaning of the options.</p>
<h3>dosDateTimeToDate(date, time)</h3><p>Converts MS-DOS <code>date</code> and <code>time</code> data into a JavaScript <code>Date</code> object.
...
<h3>Class: ZipFile</h3><p>The constructor for the class is not part of the public API.
Use <code>open()</code>, <code>fromFd()</code>, <code>fromBuffer()</code>, or <code>fromRandomAccessReader()</code> instead.</p>
<h4>Event: &quot;entry&quot;</h4><p>Callback gets <code>(entry)</code>, which is an <code>Entry</code>.
See <code>open()</code> and <code>readEntry()</code> for when this event is emitted.</p>
<h4>Event: &quot;end&quot;</h4><p>Emitted after the last <code>entry</code> event has been emitted.
See <code>open()</code> and <code>readEntry()</code> for more info on when this event is emitted.</p>
<h4>Event: &quot;close&quot;</h4><p>Emitted after the fd is actually closed.
...
<h4>Event: &quot;error&quot;</h4><p>Emitted in the case of errors with reading the zip file.
...
<h4>readEntry()</h4><p>Causes this <code>ZipFile</code> to emit an <code>entry</code> or <code>end</code> event (or an <code>error</code> event).
...
<h4>openReadStream(entry, callback)</h4><p><code>entry</code> must be an <code>Entry</code> object from this <code>ZipFile</code>.
<code>callback</code> gets <code>(err, readStream)</code>, where <code>readStream</code> is a <code>Readable Stream</code>.
...
<p>It&#39;s possible for the <code>readStream</code> it to emit errors for several reasons.
...
compared to the reported byte count from the entry&#39;s <code>uncompressedSize</code> field.
...
<p>Because of this check, clients can always trust the <code>uncompressedSize</code> field in <code>Entry</code> objects.
...
<h4>close()</h4><p>Causes all future calls to <code>openReadStream()</code> to fail,
and closes the fd after all streams created by <code>openReadStream()</code> have emitted their <code>end</code> events.</p>
...
<h4>isOpen</h4><p><code>Boolean</code>. <code>true</code> until <code>close()</code> is called; then it&#39;s <code>false</code>.</p>
<h4>entryCount</h4><p><code>Number</code>. Total number of central directory records.</p>
<h4>comment</h4><p><code>String</code>. Always decoded with <code>CP437</code> per the spec.</p>
<h3>Class: Entry</h3><p>Objects of this class represent Central Directory Records.
...
<h4>fileName</h4><p><code>String</code>.
Following the spec, the bytes for the file name are decoded with
<code>UTF-8</code> if <code>generalPurposeBitFlag &amp; 0x800</code>, otherwise with <code>CP437</code>.</p>
<p>If <code>fileName</code> would contain unsafe characters, such as an absolute path or
a relative directory, yauzl emits an error instead of an entry.</p>
<h4>extraFields</h4><p><code>Array</code> with each entry in the form <code>{id: id, data: data}</code>,
where <code>id</code> is a <code>Number</code> and <code>data</code> is a <code>Buffer</code>.
...
None of the other fields are considered significant by this library.</p>
<h4>comment</h4><p><code>String</code> decoded with the same charset as used for <code>fileName</code>.</p>
<h4>getLastModDate()</h4><p>Effectively implemented as:</p>
<pre><code class="language-js">return dosDateTimeToDate(this.lastModFileDate, this.lastModFileTime);
</code></pre>
<h3>Class: RandomAccessReader</h3><p>This class is meant to be subclassed by clients and instantiated for the <code>fromRandomAccessReader()</code> function.</p>
<p>An example implementation can be found in <code>test/test.js</code>.</p>
<h4>randomAccessReader._readStreamForRange(start, end)</h4><p>Subclasses <em>must</em> implement this method.</p>
...
<h4>randomAccessReader.read(buffer, offset, length, position, callback)</h4><p>Subclasses may implement this method.
...
<h4>randomAccessReader.close(callback)</h4><p>Subclasses may implement this method.
...
<h2>How to Avoid Crashing</h2><p>When a malformed zipfile is encountered, the default behavior is to crash (throw an exception).
...
<h2>Limitations</h2><h3>No Streaming Unzip API</h3><p>Due to the design of the .zip file format, it&#39;s impossible to interpret a .zip file from start to finish
...
<h3>Limitted ZIP64 Support</h3><p>For ZIP64, only zip files smaller than <code>8PiB</code> are supported,
...
<h3>ZIP64 Extensible Data Sector Is Ignored</h3><p>The spec does not allow zip file creators to put arbitrary data here,
...
<h3>No Multi-Disk Archive Support</h3><p>This library does not support multi-disk zip files.
...
<h3>No Encryption Support</h3><p>Currently, the presence of encryption is not even checked,
and encrypted zip files will cause undefined behavior.</p>
<h3>Local File Headers Are Ignored</h3><p>Many unzip libraries mistakenly read the Local File Header data in zip files.
...
<h3>No CRC-32 Checking</h3><p>This library provides the <code>crc32</code> field of <code>Entry</code> objects read from the Central Directory.
However, this field is not used for anything in this library.</p>
<h3>versionNeededToExtract Is Ignored</h3><p>The field <code>versionNeededToExtract</code> is ignored,
because this library doesn&#39;t support the complete zip file spec at any version,</p>
<h3>No Support For Obscure Compression Methods</h3><p>Regarding the <code>compressionMethod</code> field of <code>Entry</code> objects,
...
<h3>Data Descriptors Are Ignored</h3><p>There may or may not be Data Descriptor sections in a zip file.
This library provides no support for finding or interpreting them.</p>
<h3>Archive Extra Data Record Is Ignored</h3><p>There may or may not be an Archive Extra Data Record section in a zip file.
This library provides no support for finding or interpreting it.</p>
<h3>No Language Encoding Flag Support</h3><p>Zip files officially support charset encodings other than CP437 and UTF-8,
...
<h2>Change History</h2><ul>
<li>2.4.1<ul>
<li>Fix error handling.</li>
</ul>
</li>
<li>2.4.0<ul>
<li>Add ZIP64 support. <a href="https://github.com/thejoshwolfe/yazl/issues/6">issue #6</a></li>
<li>Add <code>lazyEntries</code> option. <a href="https://github.com/thejoshwolfe/yazl/issues/22">issue #22</a></li>
<li>Add <code>readStream.destroy()</code> method. <a href="https://github.com/thejoshwolfe/yazl/issues/26">issue #26</a></li>
<li>Add <code>fromRandomAccessReader()</code>. <a href="https://github.com/thejoshwolfe/yazl/issues/14">issue #14</a></li>
...
<li>2.3.0<ul>
<li>Check that <code>uncompressedSize</code> is correct, or else emit an error. <a href="https://github.com/thejoshwolfe/yazl/issues/13">issue #13</a></li>
</ul>
</li>
<li>2.2.1<ul>
